...
def main():
    ...
    while not game.check_ultimate_board_win():  # Main game loop
        if game.current_player == "X":  # Human's Turn
            print_board(game)  # Display the current board
            valid_move = False
            while not valid_move:
                try:
                    board_index, row, col = get_human_input()  # Function to get player input
            ...
            for move in game.get_valid_moves():
                game.make_move(*move)
                score = game.minimax(depth=4, is_maximizing=True)  # Adjust 'depth' as needed
                game.undo_move(*move)  # Assuming you have 'undo_move' implemented
                if score > best_score:
                    best_score = score
                    best_move = move

            if best_move:  # Ensure a best move was found
